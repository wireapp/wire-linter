"""Checks for RabbitMQ memory and disk alarms.
...
class RabbitmqAlarms(BaseTarget):
    """Checks RabbitMQ for active memory/disk alarms.

    Runs rabbitmqctl status on a datanode and parses the alarms
    section for active memory or disk resource alarms.
    """
    ...
    def description(self) -> str:
        ...
        return "RabbitMQ memory/disk alarms"
    ...
    def collect(self) -> bool:
        ...
        """
        self.terminal.step("Checking RabbitMQ alarms...")

        # Get the status from rabbitmqctl, it has alarm details
        result = self.run_db_command(
            self.config.databases.rabbitmq,
            "sudo rabbitmqctl status 2>/dev/null",
        )
        ...
        if header_index is not None:
            for line in lines[header_index + 1:]:
                ...
                if is_at_root:
                    ...
                    # Indented line is alarm detail (memory, disk, file_descriptors, etc.)
                    candidate = stripped[:80]
                    if candidate not in seen:
                        seen.add(candidate)
                        alarm_indicators.append(candidate)

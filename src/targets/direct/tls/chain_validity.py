"""Checks the TLS certificate chain validity for the Wire domain locally.

Uses «openssl s_client -showcerts» run locally instead of via SSH on the admin
host. The parsing logic is identical to the SSH version: we look for
«Verify return code: 0 (ok)» and count «BEGIN CERTIFICATE» markers in the output.
...
class ChainValidity(BaseTarget):
    """Checks TLS certificate chain validity for the Wire domain locally.

    Connects via openssl from the local machine and inspects the verification
    result to determine if the chain is complete.
    """

    # TLS chain validity checks connect to public domain:443
    ...
    def collect(self) -> bool:
        """Check the TLS certificate chain via local openssl s_client.

        Runs «openssl s_client -showcerts -connect <domain>:443» locally
        and inspects the output for the verify return code.
        ...
        # We pipe «Q» to stdin via echo to cleanly close the connection,
        # and combine stderr into stdout since openssl writes diagnostics there.
